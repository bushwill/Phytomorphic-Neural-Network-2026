...
def hierarchical_loss_function(pred_cost, true_cost, bp_syn, bp_probs, ep_syn, ep_probs, real_bp, real_ep):
    ...
    # Pre-calculated targets (approximate) - could be passed in better
    bp_count_target = 20.0 # Approximate average
    ep_count_target = 20.0
    
    count_loss = F.mse_loss(bp_count_pred, torch.tensor(bp_count_target, device=pred_cost.device)) + \
                 F.mse_loss(ep_count_pred, torch.tensor(ep_count_target, device=pred_cost.device))
